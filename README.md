...
Failed executing DbCommand (81ms) [Parameters=[], CommandType='Text', CommandTimeout='30']
ALTER TABLE public."Departments" ADD CONSTRAINT departments2_pkey PRIMARY KEY (id);
Npgsql.PostgresException (0x80004005): 42P16: таблица "Departments" не может иметь несколько первичных ключей
  ...
  Exception data:
    Severity: ОШИБКА
    SqlState: 42P16
    MessageText: таблица "Departments" не может иметь несколько первичных ключей
    File: index.c
    Line: 220
    Routine: index_check_primary_key
42P16: таблица "Departments" не может иметь несколько первичных ключей
