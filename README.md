...
Failed executing DbCommand (72ms) [Parameters=[], CommandType='Text', CommandTimeout='30']
    ...
    -- Вернуть PRIMARY KEY
    ALTER TABLE public."Departments" ADD CONSTRAINT departments2_pkey PRIMARY KEY (id);
Npgsql.PostgresException (0x80004005): 42601: столбец "id" отношения "Departments" является столбцом идентификации
  ...
  Exception data:
    Severity: ОШИБКА
    SqlState: 42601
    MessageText: столбец "id" отношения "Departments" является столбцом идентификации
    Hint: Выполните ALTER TABLE ... ALTER COLUMN ... DROP IDENTITY.
    File: tablecmds.c
    Line: 7939
    Routine: ATExecColumnDefault
42601: столбец "id" отношения "Departments" является столбцом идентификации
