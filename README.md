...
{
    internal class OneCService
    {
        private static string USER_CODE = "ObmenBitrix";
        private static string PASSWORD = "Qwerty1234";
        private static string ROOT_URL = "http://zuppr/ZUP83/hs";
        private static HttpClient httpClient = new HttpClient();

        public async Task<List<DepartmentDTO>> GetDepartments()
        {
            var url = $"{ROOT_URL}/GetDepartments/bitrix/";
            return await GetValueFromClient<List<DepartmentDTO>>(url) ?? [];
        }

        public async Task<List<EmployeeDTO>> GetEmployees()
        {
            var url = $"{ROOT_URL}/GetEmployee/bitrix/";
            return await GetValueFromClient<List<EmployeeDTO>>(url) ?? [];
        }

        private async Task<T?> GetValueFromClient<T>(string url)
        {
            var authString = Convert.ToBase64String(Encoding.UTF8.GetBytes($"{USER_CODE}:{PASSWORD}"));
